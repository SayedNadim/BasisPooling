...
class BasisPooling(nn.Module):
    """
    Highly optimized Basis Pooling layer that selects K representative elements from each pooling region.
    ...
    def __init__(self, kernel_size, stride=None, basis_dim=2, temperature=0.1):
        ...
        # Precompute selection patterns for each basis dimension
        # These are fixed matrices that determine which elements get selected
        # They're designed to select different elements for each basis dimension
        kernel_size_flat = self.kernel_size[0] * self.kernel_size[1]
        
        # Create basis selection patterns - these are more efficient than learnable weights
        # Different patterns for each basis dimension
        patterns = []
        for i in range(basis_dim):
            # Create different selection patterns
            if i == 0:
                # First basis focuses on maximum values (similar to max pooling)
                pattern = torch.ones(kernel_size_flat) * 0.1
            elif i == 1 and basis_dim > 1:
                # Second basis focuses on minimum values (complementary information)
                pattern = -torch.ones(kernel_size_flat) * 0.1
            else:
                # Additional bases focus on different spatial patterns
                pattern = torch.sin(torch.linspace(0, (i+1) * 3.14, kernel_size_flat))
            
            patterns.append(pattern)
        
        # Register patterns as buffer (non-learnable parameters)
        self.register_buffer('selection_patterns', torch.stack(patterns).unsqueeze(0).unsqueeze(0))
    ...
    def forward(self, x):
        ...
        # Normalize patches for better gradient flow
        patch_mean = patches.mean(dim=2, keepdim=True)
        patch_std = patches.std(dim=2, keepdim=True).clamp(min=1e-6)
        patches_normalized = (patches - patch_mean) / patch_std
        
        # Expand selection patterns to match batch and channel dimensions
        # Shape: [batch, channels, basis_dim, kernel_h*kernel_w, 1]
        patterns = self.selection_patterns.expand(
            batch_size, channels, self.basis_dim, -1, 1)
        
        # Calculate selection scores based on normalized patch values and patterns
        # Shape: [batch, channels, basis_dim, kernel_h*kernel_w, out_h*out_w]
        selection_scores = patches_normalized.unsqueeze(2) + patterns
        
        # Apply softmax with temperature to get selection weights
        # Shape: [batch, channels, basis_dim, kernel_h*kernel_w, out_h*out_w]
        weights = F.softmax(selection_scores / self.temperature, dim=3)
        
        # Apply weights to original patches to get basis vectors
